...
//   printf("what is the fd in request handle? %d\n", fd);
   Rio_readinitb(&rio, fd);
  // printf("after the read init inside the request handle\n");
   Rio_readlineb(&rio, buf, MAXLINE);
 //  printf("after read a line of rio?\n");
   sscanf(buf, "%s %s %s", method, uri, version);

   printf("%s %s %s\n", method, uri, version);
  // printf("finished reading everyting!\n");
