...
clone(void (*fcn)(void*), void *arg, void *stack)
{
  ...
  //checks to see if stack right size
...
int
condsleep(void* chan, struct spinlock *lk)
{
  //sleep((void*)num, &);
   if(proc == 0)
    panic("sleep");
...
int
condwakeup(void* chan)
{
  acquire(&ptable.lock);
  struct proc *p;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if(p->state == SLEEPING && p->chanId == *(int*)chan)
      p->state = RUNNABLE;
  release(&ptable.lock);
  return 0;
}
