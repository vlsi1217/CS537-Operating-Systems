...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  char *mem;
  uint a;
  if(oldsz == USERTOP - proc->ss - PGSIZE)
  {
  	if(oldsz < proc->sz + PGSIZE)
	{
		return 0;
	}
  }
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    int status;
    if(i == 0) {
	status = mappages(d, (void*)i, PGSIZE, PADDR(mem), 0);
    }
    else
    {
	status = mappages(d, (void*)i, PGSIZE, PADDR(mem), PTE_W|PTE_U);
    }
    if(status == -1)
	goto bad;
  }
  //copy the end stack to another process
  for(i = USERTOP - proc->ss; i < USERTOP; i+=PGSIZE)
  {
  	if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
   		panic("copyuvm: pte should exist");
  	if(!(*pte & PTE_P))
