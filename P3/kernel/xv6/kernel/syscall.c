...
fetchint(struct proc *p, uint addr, int *ip)
{
  ...
  if((addr >= p->sz && addr < p->tf->esp) || addr > USERTOP || (addr+4 > p->sz && addr - 4 < p->tf->esp) || addr + 4 > USERTOP)
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if((uint) i < PGSIZE || ((uint)i >= proc->sz && (uint)i < USERTOP - proc->ss) || (uint) i > USERTOP || ((uint)i+size > proc->sz && (uint)i - size < USERTOP - proc->ss))
    return -1;
  *pp = (char*)i;
  return 0;
}
