...
//   + Files: inode allocator, ing, writing, metadata.
...
stati(struct inode *ip, struct stat *st)
{
  ...
  if(ip->type == T_CHECKED)
  {
    ...
    for(i = 0; i < (NDIRECT + 1); i++)
    {
      ...
      {
        uint temp = ip->addrs[i] & 0xff000000;
        temp = temp >> 24;
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    bmapi = bmap(ip, off/BSIZE);
    bp1 = bread(ip->dev, bmapi);
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp1->data + off%BSIZE, m);
    ...
    if(ip->type == T_CHECKED)
    {
      ...
      if(bn < NDIRECT)
      {
        checksum1 = ip->addrs[bn] & 0xff000000;
        checksum1 = (checksum1 >> 24);
      }
      bn -= NDIRECT;
      if(bn < NINDIRECT){
